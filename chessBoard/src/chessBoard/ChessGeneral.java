...
public class ChessGeneral
{
	...
	public ChessGeneral (Image bImage,Image Image,int row,int column,int flag,String typeName)
	{
		this.b_Image=bImage;
		this.Image=Image;
		this.row=row;
	...
	public void ChessMove(ChessCoordinate[][]ccd,int prow,int pcolumn,int arow,int acolumn)
	{
		...
		if(Math.abs(arow-prow)==1&&pcolumn==acolumn&&arow<=2&&acolumn<=5&&acolumn>=3)
		{
		...
		if(arow==prow&&Math.abs(acolumn-pcolumn)==1&&arow<=2&&acolumn<=5&&acolumn>=3)
		{
			if(ccd[arow][acolumn].getBoardFlag()==1)
			{
				if(ccd[arow][acolumn].getchessFlag()==flag)
	...
	//rea chess
		if(Math.abs(arow-prow)==1&&acolumn==pcolumn&&arow>=7&&acolumn<=5&&acolumn>=3)
		{
			if(ccd[arow][acolumn].getBoardFlag()==1)
			{
				if(ccd[arow][acolumn].getchessFlag()==flag)
		...
		if(arow==prow&&Math.abs(acolumn-pcolumn)==1&&arow>=7&&acolumn<=5&&acolumn>=3)
		{
			if(ccd[arow][acolumn].getBoardFlag()==1)
			{
				if(ccd[arow][acolumn].getchessFlag()==flag)
